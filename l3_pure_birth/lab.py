...
def main():
    ...
    for lambda_ in lambdas:
        ...
        for pb in pbs:
            pb.model(modelling_states)
        max_transition = np.max([np.max(pb.transitions[:-1]) for pb in pbs])
        step = max_transition / step_factor
        all_transitions = np.arange(0, max_transition, step)
        all_states = []
            ...
            for pb in pbs:
                for index, pb_transition in enumerate(pb.transitions):
                    if pb_transition > transition:
                        all_states[len(all_states) - 1].append(index - 1)
                        break
        ...
        logging.info('Generating plot for state probabilites, M and D...')
